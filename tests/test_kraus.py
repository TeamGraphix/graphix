...
class TestKraus(unittest.TestCase):
    ...
    def test_generate_dephasing_kraus_success(self):
        p = 0.5
        qarg = 1
        dephase = generate_dephasing_kraus(p, qarg)
        print(dephase)
        self.assertEqual(len(dephase), 20)
        np.testing.assert_array_equal(dephase[0].data, np.asarray(np.sqrt(1 - p) * np.eye(2), dtype=complex))
        np.testing.assert_array_equal(dephase[1].data, np.asarray(np.sqrt(p) * np.diag([1, -1]), dtype=complex))
        self.assertEqual(dephase[0].qarg, qarg)
        self.assertEqual(dephase[1].qarg, qarg)
