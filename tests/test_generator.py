...
class TestGenerator(unittest.TestCase):
    def test_pattern_generation_determinism_flow(self):
        graph = nx.Graph(
            [(0, 3), (1, 4), (2, 5), (1, 3), (2, 4), (3, 6), (4, 7), (5, 8)]
        )
        inputs = {0, 1, 2}
        outputs = {6, 7, 8}
        ...
        for _ in range(repeats):
            pattern = generate_from_graph(
                graph, angles, list(inputs), list(outputs), meas_planes=meas_planes
            )
            pattern.standardize()
        ...
        for i, j in combinations:
            inner_product = np.dot(
                results[i].flatten(), results[j].flatten().conjugate()
            )
    ...
    def test_pattern_generation_flow(self):
        ...
        pattern2 = generate_from_graph(
            g, angles, input, pattern.output_nodes, meas_planes
        )
