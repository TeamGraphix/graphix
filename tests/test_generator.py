...
def example_flow(rng: Generator) -> Pattern:
    ...
    outputs = [7, 6, 8]
    angles = dict(zip(range(6), (2 * rng.random(6)).tolist(), strict=True))
...
def example_graph_pflow(rng: Generator) -> OpenGraph:
    ...
    meas = {
        i: Measurement(angle, plane)
        for (i, angle), plane in zip(meas_angles.items(), meas_planes.values(), strict=True)
    }
