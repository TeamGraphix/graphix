...
class TestPattern:
    ...
    def test_compose_6(self, fx_bg: PCG64, jumps: int) -> None:
        ...
        p_compose, _ = p1.compose(p2, mapping=dict(zip(p2.input_nodes, p1.output_nodes, strict=False)), preserve_mapping=True)
        p.minimize_space()
...
class TestMCOps:
    ...
    def test_arbitrary_inputs(self, fx_rng: Generator, nqb: int, rand_circ: Circuit, backend: str) -> None:
        ...
        states = [PlanarState(plane=i, angle=j) for i, j in zip(rand_planes, rand_angles, strict=False)]
...
def assert_equal_edge(edge: Sequence[int], ref: Sequence[int]) -> bool:
    return any(all(ei == ri for ei, ri in zip(edge, other, strict=False)) for other in (ref, reversed(ref)))
