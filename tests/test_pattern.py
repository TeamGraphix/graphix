...
def test_draw_pattern_measure() -> None:
    ...
    assert str(p) == "N(1) N(2) N(3) N(10) E(1,2) M(1,YZ,π/2) M(2,XZ,-π/4) M(10,XZ,-π/4) {2}[M(3,0.31)]{1,10}"
    assert p.to_unicode() == "N₁ N₂ N₃ N₁₀ E₁₋₂ M₁(YZ,π/2) M₂(XZ,-π/4) M₁₀(XZ,-π/4) ₂[M₃(0.31)]¹,¹⁰"
    assert (
        p.to_latex()
        == r"\(N_{1}\,N_{2}\,N_{3}\,N_{10}\,E_{1,2}\,M_{1}^{YZ,\frac{\pi}{2}}\,M_{2}^{XZ,-\frac{\pi}{4}}\,M_{10}^{XZ,-\frac{\pi}{4}}\,{}_2[M_{3}^{0.31}]^{1,10}\)"
    )
