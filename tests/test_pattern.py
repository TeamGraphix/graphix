...
def test_draw_pattern_measure() -> None:
    ...
    assert str(p) == "N(1) N(2) N(3) E(1,2) M(1,YZ,π/2) M(2,XZ,-π/4) [1]M(3,0.31)[2]"
    assert p.to_unicode() == "N₁ N₂ N₃ E₁₋₂ M₁(YZ,π/2) M₂(XZ,-π/4) ₁M₃(0.31)²"
    d = p.to_latex()
    print(d)
    assert p.to_latex() == r"\(N_{1} N_{2} N_{3} E_{1,2} M_{1}^{YZ,\frac{\π}{2}} M_{2}^{XZ,-\frac{\π}{4}} {}_1[M_{3}^{0.31}]^{2}\)"