...
underlying open graph, which need not be unique).
"""
...
from tests.test_flow_core import prepare_test_xzcorrections
...
def test_extract_pauli_flow_matches_reference_flows() -> None:
    """Reconstructed ``p`` must match the reference correction function, not only round-trip XZ."""
    for test_case in prepare_test_xzcorrections():
        flow = test_case.flow
        xz = flow.to_corrections()
        pf = xz.to_pauli_flow()
        assert pf.correction_function == flow.correction_function
...
def test_draw_flow_from_pattern_skips_opengraph_for_pauli_measurements() -> None:
    """``Pattern.draw`` must extract Pauli flow from corrections without ``downcast_bloch``."""
    pattern = _pauli_pattern()
    mock_gv = MagicMock()
    with (
        patch("graphix.visualization.GraphVisualizer.from_flow", return_value=mock_gv) as from_flow,
        patch.object(pattern, "extract_opengraph") as extract_opengraph,
    ):
        pattern.draw(annotations=DrawPatternAnnotations.Flow, flow_from_pattern=True)
        extract_opengraph.assert_not_called()
        flow = from_flow.call_args.kwargs["flow"]
    assert flow.correction_function[0] == frozenset({1, 3})
