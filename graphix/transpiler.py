...
class Circuit():
        ...
        for instr in self.instruction:
            if instr[0] == 'CNOT':
                ...
                out[instr[1][0]], out[instr[1][1]], seq =\
                    self._cnot_command(out[instr[1][0]], out[instr[1][1]], ancilla)
    ...
    def _commute_with_cnot(self, target):
        ...
        if self._instr[target][0] == 'XC' and\
                self._instr[target][1] == self._instr[target + 1][1][0]:  # control
            new_cmd = ['XC', self._instr[target + 1][1][1], self._instr[target][2]]
            self._commute_with_following(target)
            self._instr.insert(target + 1, new_cmd)
            return target + 1
        elif self._instr[target][0] == 'ZC' and\
                self._instr[target][1] == self._instr[target + 1][1][1]:  # target
