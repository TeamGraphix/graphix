...
class Circuit:
    ...
    def standardize_and_transpile(self) -> Pattern:
        """gate-to-MBQC transpile function.
    ...
    def _commute_with_cnot(self, target: int) -> int:
    ...
    def _commute_with_H(self, target: int) -> None:
        assert self._instr[target][0] in ["XC", "ZC"]
    ...
    def _commute_with_S(self, target: int) -> int:
        assert self._instr[target][0] in ["XC", "ZC"]
    ...
    def _commute_with_Rx(self, target: int) -> int:
        assert self._instr[target][0] in ["XC", "ZC"]
    ...
    def _commute_with_Ry(self, target: int) -> None:
        assert self._instr[target][0] in ["XC", "ZC"]
    ...
    def _commute_with_Rz(self, target: int) -> None:
        assert self._instr[target][0] in ["XC", "ZC"]
    ...
    def _find_byproduct_to_move(self, rev: bool = False, skipnum: int = 0) -> int:
    ...
    def _move_byproduct_to_right(self) -> None:
        ...
        moved: int = 0  # number of moved op
    ...
    @classmethod
    def _sort_outputs(self, pattern: Pattern, output_nodes):
    ...
    def simulate_statevector(self, input_state=None) -> Statevec:
