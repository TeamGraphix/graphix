...
class PauliFlow(Generic[_M_co]):
    ...
    def check_well_formed(self) -> None:
        ...
        for layer in reversed(self.partial_order_layers[1:]):
            ...
            for node in layer:
                ...
                meas = self.measurement_label(node)
    ...
    def measurement_label(self, node: int) -> Plane | Axis:
...
class GFlow(PauliFlow[_PM_co], Generic[_PM_co]):
    ...
    def check_well_formed(self) -> None:
        ...
        for layer in reversed(self.partial_order_layers[1:]):
            ...
            for node in layer:
                ...
                plane = self.measurement_label(node)
    ...
    @override
    def measurement_label(self, node: int) -> Plane:
        """Get the measurement label of a given node in the open graph.
