...
_C0: npt.NDArray = np.array([[1, 0], [0, 1]])  # identity
_C1: npt.NDArray = np.array([[0, 1], [1, 0]])  # X
_C2: npt.NDArray = np.array([[0, -1j], [1j, 0]])  # Y
_C3: npt.NDArray = np.array([[1, 0], [0, -1]])  # Z
_C4: npt.NDArray = np.array([[1, 0], [0, 1j]])  # S = \sqrt{Z}
_C5: npt.NDArray = np.array([[1, 0], [0, -1j]])  # S dagger
_C6: npt.NDArray = np.array([[1, 1], [1, -1]]) / np.sqrt(2)  # Hadamard
_C7: npt.NDArray = np.array([[1, -1j], [-1j, 1]]) / np.sqrt(2)  # \sqrt{iX}
_C8: npt.NDArray = np.array([[1, -1], [1, 1]]) / np.sqrt(2)  # \sqrt{iY}
_C9: npt.NDArray = np.array([[0, 1 - 1j], [-1 - 1j, 0]]) / np.sqrt(2)  # sqrt{I}
_C10: npt.NDArray = np.array([[0, -1 - 1j], [1 - 1j, 0]]) / np.sqrt(2)  # sqrt{-I}
_C11: npt.NDArray = np.array([[1, -1], [-1, -1]]) / np.sqrt(2)  # sqrt{I}
_C12: npt.NDArray = np.array([[-1, -1], [1, -1]]) / np.sqrt(2)  # sqrt{-iY}
_C13: npt.NDArray = np.array([[1j, -1], [1, -1j]]) / np.sqrt(2)  # sqrt{-I}
_C14: npt.NDArray = np.array([[1j, 1], [-1, -1j]]) / np.sqrt(2)  # sqrt{-I}
_C15: npt.NDArray = np.array([[-1, -1j], [-1j, -1]]) / np.sqrt(2)  # sqrt{-iX}
_C16: npt.NDArray = np.array([[-1 + 1j, 1 + 1j], [-1 + 1j, -1 - 1j]]) / 2  # I^(1/3)
_C17: npt.NDArray = np.array([[-1 + 1j, -1 - 1j], [1 - 1j, -1 - 1j]]) / 2  # I^(1/3)
_C18: npt.NDArray = np.array([[1 + 1j, 1 - 1j], [-1 - 1j, 1 - 1j]]) / 2  # I^(1/3)
_C19: npt.NDArray = np.array([[-1 - 1j, 1 - 1j], [-1 - 1j, -1 + 1j]]) / 2  # I^(1/3)
_C20: npt.NDArray = np.array([[-1 - 1j, -1 - 1j], [1 - 1j, -1 + 1j]]) / 2  # I^(1/3)
_C21: npt.NDArray = np.array([[-1 + 1j, -1 + 1j], [1 + 1j, -1 - 1j]]) / 2  # I^(1/3)
_C22: npt.NDArray = np.array([[1 + 1j, -1 - 1j], [1 - 1j, 1 - 1j]]) / 2  # I^(1/3)
_C23: npt.NDArray = np.array([[-1 + 1j, 1 - 1j], [-1 - 1j, -1 - 1j]]) / 2  # I^(1/3)
