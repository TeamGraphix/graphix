...
class Pattern:
    ...
    def get_local_pattern(self):
        ...
        node_prop = {input_: fresh_node() for input_ in self.__input_nodes}
        morder = []
        ...
        with open(filename + ".qasm", "w") as file:
            ...
            if self.results != {}:
                for id_ in self.results:
                    ...
                    file.write("// measurement result of qubit q" + str(id_) + "\n")
                    file.write("bit c" + str(id_) + " = " + str(res) + ";\n")
                    file.write("\n")
...
def assert_permutation(original, user):
    ...
    for node in user:
        if node in node_set:
            ...
            raise AssertionError(f"{node} appears twice")
