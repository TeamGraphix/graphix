...
class RustworkxGraphState(BaseGraphState):
    ...
    def add_nodes_from(self, nodes: list[int]):
        ...
        """
        self._graph.add_nodes_from([(i, {"loop": False, "sign": False, "hollow": False}) for i in nodes])
    ...
    def add_edges_from(self, edges):
        ...
        """
        self._graph.add_edges_from_no_data(edges)
        for nidx in self._graph.node_indexes():
            if "loop" not in self._graph[nidx]:
                self._graph[nidx]["loop"] = False  # True for having loop
                self._graph[nidx]["sign"] = False  # True for minus
                self._graph[nidx]["hollow"] = False  # True for hollow node
    ...
    def get_vops(self):
        ...
        for nidx in self._graph.node_indexes():
            vop = 0
            if self._graph[nidx]["sign"]:
                vop = CLIFFORD_MUL[3, vop]
            if self._graph[nidx]["loop"]:
                vop = CLIFFORD_MUL[4, vop]
            if self._graph[nidx]["hollow"]:
                vop = CLIFFORD_MUL[6, vop]
            node_data = self._graph[nidx]
            vops[node_data[0]] = vop
        return vops
    ...
    def flip_fill(self, node):
        ...
        for nidx in self._graph.node_indexes():
            if self._graph[nidx][0] == node:
                self._graph[nidx]["hollow"] = not self._graph[nidx]["hollow"]
