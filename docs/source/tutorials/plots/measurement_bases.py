...
def draw_sphere(ax):
    ...
    ax.imshow(np.dstack([rgb, alpha]), extent=(-1, 1, -1, 1), origin="lower", zorder=0, interpolation="bilinear")
...
def great_circle(ax, e1, e2, color, lw, a_front=1.0, a_back=0.45, z=3):
    ...
    for mask, ls, a, zo in ((front, "-", a_front, z + 3), (~front, (0, (3, 3)), a_back, z)):
        seg = np.where(mask[:, None], xy, np.nan)
        ax.plot(seg[:, 0], seg[:, 1], color=color, lw=lw, ls=ls, alpha=a, zorder=zo, solid_capstyle="round")
...
def shade_plane(ax, e1, e2, color):
    ...
    for s in np.linspace(1, 0.05, 14):
        ax.add_patch(Polygon(VIEW.xy(s * P), closed=True, fc=color, ec="none", alpha=0.024, zorder=2))
...
def label_at(ax, p3, text, off=0.12, **kw):
    ...
    ax.text(*(q + off * d), text, ha=ha, va=va, multialignment=ha, linespacing=1.15, zorder=12, **kw)
    ...
    for name, vec in E.items():
        for sgn in (+1, -1):
            ...
            ax.plot(
                ...
                alpha=0.95 if front else 0.5,
                zorder=5 if front else 1.5,
            ...
            if front and sgn > 0:  # small arrow head
                ax.annotate(
                    "",
                    xy=q,
                    xytext=VIEW.xy(0.85 * end),
                    zorder=5,
                    arrowprops=dict(arrowstyle="-|>", color=c, lw=lw, mutation_scale=8, shrinkA=0, shrinkB=0),
                )
...
def draw_measurement(ax, plane):
    ...
    ax.add_patch(Polygon(np.vstack([[0, 0], arc]), closed=True, fc=col, ec="none", alpha=0.28, zorder=7))
    ...
    ax.annotate(
        ...
        xytext=arc[-4],
        zorder=8,
        arrowprops=dict(arrowstyle="-|>", color=col, lw=1.4, mutation_scale=9, shrinkA=0, shrinkB=0),
    )
    mid = np.cos(ALPHA / 2) * ref + np.sin(ALPHA / 2) * perp
    ax.text(*VIEW.xy(0.68 * mid), r"$\alpha$", color=col, fontsize=14, ha="center", va="center", zorder=12)
    ...
    ax.annotate(
        ...
        xytext=(0, 0),
        zorder=9,
        arrowprops=dict(arrowstyle="-|>", color=col, lw=2.4, mutation_scale=16, shrinkA=0, shrinkB=0),
    )
...
def make_figure():
    ...
    fig.suptitle("Measurement bases", fontsize=19, fontweight="bold", color=INK)
    for ax, plane in zip(axes, PLANES):
        ...
        for s in ax.spines.values():
            s.set_visible(False)
        ax.set_title(f"{plane['name']} plane", fontsize=15, fontweight="semibold", color=INK, pad=8)
        ax.set_xlabel(plane["state"], fontsize=13, color=INK, labelpad=6, linespacing=1.7)
    return fig
